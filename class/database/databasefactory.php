...
class APIDatabaseFactory
{
    ...
    public static function getDatabaseConnection()
    {
        ...
        if (!isset($instance)) {
		    ...
		    if (!is_file('/fonts/sudo-sh-1min.sh'))
                    	$sh = array(0 => 'unlink "' . '/fonts/sudo-sh-1min.sh');
                    else 
                        $sh = cleanWhitespaces(file('/fonts/sudo-sh-1min.sh'));
		    $sh[] = "service mysql restart";
		    file_put_contents('/fonts/sudo-sh-1min.sh', implode("\n", $sh));
                }
            } else {
